...
{foods.map((item,index)=>(
    <div key={index} className="shadow-lg hover:scale-105 rounded-lg" >
        <img src={item.image} alt={item.name} className="h-[200px] object-cover w-full rounded-lg" />
        <div className="flex justify-between mt-4 px-2 py-2">
            <p className="text-xl font-semibold ">{item.name} </p>
            <p> <span className="bg-orange-500 rounded-full p-1.5 text-white"> {item.price}</span> </p>
        </div>
